...
--
-- Changelog:
-- 1.0.0 - Character and cursed-energy definitions
--==============================================================--
local Database = {}
...
function Database:ResolveFromBackpack(backpack)
    ...
    for _, item in ipairs(backpack:GetChildren()) do
        table.insert(names, self:Normalize(item.Name))
    end
    local priority = {
        {"hero hunter: cosmic", "cosmic"}, {"hero hunter: monst", "monst", "monster"},
        {"destructive cyborg", "cyborg"}, {"deadly ninja", "ninja"}, {"brutal demon", "demon"},
        {"blade master", "blade"}, {"wild psychic", "psychic"}, {"martial artist", "martial"},
        {"tech prodigy", "tech"}, {"undying hero", "undying"}, {"hero hunter", "hunter"},
    ...
    for _, row in ipairs(priority) do
        for _, needle in ipairs(row) do
            for _, itemName in ipairs(names) do
                if itemName:find(needle, 1, true) then return self.Characters[row[1]] end
            end
        end
    end
    return nil
