...
function CameraEffects:Init(Config)
    ...
    self.Connection = RunService.RenderStepped:Connect(function(dt)
        ...
        if self.Shake > 0 and Config.Settings.Camera.ShakeEnabled then
            ...
            self.Shake = math.max(0, self.Shake - dt * amount * 7)
        end
    end)
end
...
function CameraEffects:Shake(strength, duration)
    ...
    if duration then
        local token = self.Token + 1; self.Token = token
        task.delay(duration, function() if self.Token == token then self.Shake = 0 end end)
    end
end

function CameraEffects:CriticalShake()
    self:Shake(self.Config.Effects.CriticalShakeStrength, 0.3)
end
...
function CameraEffects:SlowMotion(duration)
    ...
    local tracks = humanoid and humanoid:GetPlayingAnimationTracks() or {}
    local previousSpeeds = {}
    for _, track in ipairs(tracks) do
        previousSpeeds[track] = track.Speed
        pcall(function() track:AdjustSpeed(math.max(0.05, track.Speed * 0.08)) end)
    end
    local token = os.clock()
    self.SlowToken = token
    task.delay(duration, function()
        if self.SlowToken ~= token then return end
        for track, speed in pairs(previousSpeeds) do
            if track and track.Parent then pcall(function() track:AdjustSpeed(speed) end) end
        end
    end)
end
